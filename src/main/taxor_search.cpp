...
void set_up_subparser_layout(seqan3::argument_parser & parser, taxor::search::configuration & config)
{
    ...
    parser.add_option(config.stashmap_file, 
                      '\0', "stashmap", "file containing stashed kmers from FUSOR build");
...
void search_single(hixf::search_arguments & arguments, taxor_index<hixf_t> && index, hixf::sync_out &outstrm, std::string const & stashmap_file)
{
    ...
    if (!stashmap_file.empty())
    {
        taxor::stash_map_type tmp_stash{};
        taxor::load_stash_map(stashmap_file, tmp_stash);
    ...
    auto worker = [&](size_t const start, size_t const end)
    {
        ...
        {
            if (has_stash)
                return index.ixf().membership_agent(&stash_map);
            else
...
void search_hixf(taxor::search::configuration const config)
{
    ...
    //	    search_args.out_file = config.report_file;
            ...
            auto index = taxor_index<hixf_t>{};
            search_single(search_args, std::move(index), synced_out, config.stashmap_file);
        }
    }
}
