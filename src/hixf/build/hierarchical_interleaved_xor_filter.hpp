...
public:
    ...
    //!\brief The type of an individual filter (XOR, 3-way BFF, or 4-way BFF).
    using ixf_t = std::variant<seqan3::interleaved_xor_filter<FingerprintType>, 
                               seqan3::interleaved_3way_binary_fuse_filter<FingerprintType>,
                               seqan3::interleaved_4way_binary_fuse_filter<FingerprintType>>;
    ...
    class membership_agent membership_agent(ankerl::unordered_dense::map<int64_t, std::vector<std::pair<uint64_t, uint32_t>>> const * stash_ptr) const
    {
        using AgentType = class membership_agent;
        return AgentType{*this, stash_ptr};
    }
    ...
    template <std::integral value_t = uint32_t>
    counting_agent_type<value_t> counting_agent(ankerl::unordered_dense::map<int64_t, std::vector<std::pair<uint64_t, uint32_t>>> const * stash_ptr) const
    {
        return counting_agent_type<value_t>{*this, stash_ptr};
    }
...
private:
    ...
    //!\brief Pointer to inverted stash map, if available
    ankerl::unordered_dense::map<int64_t, std::vector<std::pair<uint64_t, uint32_t>>> const * stash_map_ptr{nullptr};

    //!\brief Query kmer count map built during bulk_contains
    ankerl::unordered_dense::map<uint64_t, uint32_t> query_kmer_counts{};
    ...
    {
        std::visit([&](auto const & filter)
        {
            auto agent = filter.template counting_agent<uint32_t>();

            auto const & result = agent.bulk_count(values);
            
            // Re-hydrate stash hits for current ixf_idx before evaluating rules
            ankerl::unordered_dense::map<uint32_t, uint32_t> local_stash{};
            if (stash_map_ptr != nullptr) {
                auto it = stash_map_ptr->find(ixf_idx);
                if (it != stash_map_ptr->end()) {
                    for (auto const& [stashed_kmer, bin_idx] : it->second) {
                        auto q_it = query_kmer_counts.find(stashed_kmer);
                        if (q_it != query_kmer_counts.end()) {
                            local_stash[bin_idx] += q_it->second;
                        }
                    }
                }
            }
            ...
            {
                sum += result[bin];
                auto stash_it = local_stash.find(bin);
                if (stash_it != local_stash.end())
                    sum += stash_it->second;
...
public:
     ...
     */
    explicit membership_agent(hixf_t const & hixf) : hixf_ptr(std::addressof(hixf)), stash_map_ptr(nullptr)
    {}

    explicit membership_agent(hixf_t const & hixf, ankerl::unordered_dense::map<int64_t, std::vector<std::pair<uint64_t, uint32_t>>> const * stash_ptr) 
        : hixf_ptr(std::addressof(hixf)), stash_map_ptr(stash_ptr)
    {}
    //!\}
...
private:
    //!\brief The type of the augmented hierarchical_interleaved_bloom_filter.
    using hixf_t = hierarchical_interleaved_xor_filter<FingerprintType>;
    using TIXFAgent = typename seqan3::interleaved_xor_filter<FingerprintType>::template counting_agent_type<value_t>;
    ...
    //!\brief Query kmer count map built during bulk_count
    ankerl::unordered_dense::map<uint64_t, uint32_t> query_kmer_counts{};
    ...
    void bulk_count_impl(value_range_t && values, int64_t const ixf_idx, size_t const threshold)
    {
        std::visit([&](auto const & filter)
        {
            auto agent = filter.template counting_agent<value_t>();
            auto const & result = agent.bulk_count(values);

            // Re-hydrate stash hits for current ixf_idx before evaluating rules
            ankerl::unordered_dense::map<uint32_t, value_t> local_stash{};
            if (stash_map_ptr != nullptr) {
                auto it = stash_map_ptr->find(ixf_idx);
                if (it != stash_map_ptr->end()) {
                    for (auto const& [stashed_kmer, bin_idx] : it->second) {
                        auto q_it = query_kmer_counts.find(stashed_kmer);
                        if (q_it != query_kmer_counts.end()) {
                            local_stash[bin_idx] += static_cast<value_t>(q_it->second);
                        }
                    }
                }
            }
...
public:
    ...
    explicit counting_agent_type(hixf_t const & hixf) :
        ...
        stash_map_ptr(nullptr),
        result_buffer(hixf_ptr->user_bins.num_user_bins())
    {}

    explicit counting_agent_type(hixf_t const & hixf, ankerl::unordered_dense::map<int64_t, std::vector<std::pair<uint64_t, uint32_t>>> const * stash_ptr) :
        hixf_ptr(std::addressof(hixf)),
        stash_map_ptr(stash_ptr),
        result_buffer(hixf_ptr->user_bins.num_user_bins())
    {}
    //!\}
