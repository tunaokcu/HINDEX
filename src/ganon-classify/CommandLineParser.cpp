...
std::optional< Config > CommandLineParser::parse( int argc, char** argv )
{
    ...
    options.add_options()
        ...
        ( "i,hixf", "hixf file[s] from ganon-build (comma-separated)", cxxopts::value< std::vector< std::string > >() )
        ( "stashmap", "stashmap file[s] (comma-separated)", cxxopts::value< std::vector< std::string > >() )
    ...
    if ( args.count( "hixf" ) )
        config.hixf = args["hixf"].as< std::vector< std::string > >();
    if ( args.count( "stashmap" ) )
        config.stashmap = args["stashmap"].as< std::vector< std::string > >();
    if ( args.count( "tax" ) )
