...
public:
    ...
    std::vector< std::string > hixf;
    std::vector< std::string > tax;
    std::vector< std::string > stashmap;
    ...
    bool validate()
    {
        ...
        if ( hixf.size() == 0 || ( paired_reads.size() == 0 && single_reads.size() == 0 ) )
        {
            std::cerr << "--hixf and --[single|paired]-reads are mandatory" << std::endl;
            return false;
        }
    ...
    bool validate_hierarchy()
    {
        ...
        if ( tax.size() > 0 && hixf.size() != tax.size() )
        {
            std::cerr << "The number of files provided with --hixf and --tax should match" << std::endl;
            return false;
        }

        // One hierarchy value for multiple hierarchies
        if ( hierarchy_labels.size() == 1 && hixf.size() > 1 )
        {
            for ( uint16_t b = 1; b < hixf.size(); ++b )
            {
                hierarchy_labels.push_back( hierarchy_labels[0] );
            }
        }
        else if ( hierarchy_labels.size() != hixf.size() )
        {
            std::cerr << "--hierarchy does not match with the number of --hixf and --tax" << std::endl;
            return false;
        }

        // If only one max error was given, repeat it for every filter
        if ( rel_cutoff.size() == 1 && hixf.size() > 1 )
        {
            for ( uint16_t b = 1; b < hixf.size(); ++b )
            {
        ...
        else if ( rel_cutoff.size() != hixf.size() )
        {
