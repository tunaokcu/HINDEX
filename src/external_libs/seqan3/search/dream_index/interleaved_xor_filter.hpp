...
private:
    ...
    */
                      ...
                      int alone_pos)
    {
        ...
        while (reverse_order_pos + stash_count < size)
        {
            if (alone_pos == 0) 
            {
            ...
            if (alone_pos == 0) {
                ...
                while (key_cursor < size) {
                    if (key_status[key_cursor] == 0) {
                        ...
                        uint32_t h0 = getHashFromHash(hash, 0, block_length);
                        uint32_t h1 = getHashFromHash(hash, 1, block_length);
                        uint32_t h2 = getHashFromHash(hash, 2, block_length);
                        
                        if (t2vals[h0].t2count >= 1 && t2vals[h1].t2count >= 1 && t2vals[h2].t2count >= 1) {
                            ...
                            t2vals[h0].t2count--;
                            t2vals[h0].t2 ^= hash;
                            if (t2vals[h0].t2count == 1) alone_positions[alone_pos++] = h0;
                            
                            t2vals[h1].t2count--;
                            t2vals[h1].t2 ^= hash;
                            if (t2vals[h1].t2count == 1) alone_positions[alone_pos++] = h1;
                            
                            t2vals[h2].t2count--;
                            t2vals[h2].t2 ^= hash;
                            if (t2vals[h2].t2count == 1) alone_positions[alone_pos++] = h2;
                ...
                if (!found_victim && (reverse_order_pos + stash_count != size)) {
                    construction_failed = true;
                    break;
                }
...
public:
    ...
    bool operator==(interleaved_xor_filter const & rhs) const noexcept
    {
    ...
    bool operator<(interleaved_xor_filter const & rhs) const noexcept
    {
        ...
        return std::lexicographical_compare(data.begin(), data.end(), rhs.data.begin(), rhs.data.end());
    }

    /*!\brief Returns the number of elements in the stash.
     * \returns number of stashed elements across all bins
     */
    ...
    /*!\brief Returns a reference to the bin stashes
     * \returns A const reference to the vector of vectors containing stashed elements
     */
     ...
     * \returns true if the key is in the stash, false otherwise
     */
    inline bool stash_contains(size_t bin, uint64_t key) const
    {
        if (bin >= bin_stashes.size()) return false;
        const auto& stash = bin_stashes[bin];
        return std::find(stash.begin(), stash.end(), key) != stash.end();
    }
    ...
    bool add_bin_elements(size_t bin, std::vector<size_t>& elements)
    {
        ...
        std::vector<uint8_t> key_status(elements.size(), 0);
        size_t stash_count = 0;
        bool construction_failed = false;
        ...
        rev_order.resize(reverse_order_pos);
        reverse_h.resize(reverse_order_pos);
