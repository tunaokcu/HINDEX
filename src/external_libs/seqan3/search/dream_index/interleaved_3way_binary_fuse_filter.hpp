...
private:
     ...
     * 
     */
    inline constexpr uint64_t murmur64(uint64_t h, uint64_t seed_val) const
    {
        h += seed_val;
        h ^= h >> 33;
    ...
    inline constexpr uint64_t murmur64(uint64_t h) const { return murmur64(h, this->seed); }
     ...
     *  
     */
    int find_alone_positions(const std::vector<size_t>& elements, std::vector<t2val_t>& t2vals, sdsl::int_vector<>& alone_positions, uint64_t seed_val) const
    {
...
public:
                       ...
                       std::vector<uint8_t>& out_reverse_h, size_t& out_stacksize) const
    {
        ...
        static thread_local sdsl::int_vector<0> alone_positions;
