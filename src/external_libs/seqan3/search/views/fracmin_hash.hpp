...
//!\brief strong_type for the scaling factor.
//!\ingroup search_views
struct scale_factor : seqan3::detail::strong_type<double, scale_factor>
{
    using seqan3::detail::strong_type<double, scale_factor>::strong_type;
};
} // namespace seqan3
...
struct fracmin_hash_fn
{
    ...
    * \param[in] scale_factor   The windows size to use.
    * \throws std::invalid_argument if the size of the shape is greater than the `window_size`.
    * \returns               A range of converted elements.
    */
    constexpr auto operator()(shape const & shape, window_size const window_size, scale_factor const scale_factor) const
    {
        return seqan3::detail::adaptor_from_functor{*this, shape, window_size, scale_factor};
    }
    ...
    * \returns               A range of converted elements.
    */
    constexpr auto operator()(shape const & shape, window_size const window_size, scale_factor const scale_factor, seed const seed) const
    {
        return seqan3::detail::adaptor_from_functor{*this, shape, window_size, scale_factor, seed};
    }
                              ...
                              seed const seed = seqan3::seed{0x8F3F73B5CF1C9ADE}) const
    {
        ...
        return seqan3::detail::minimiser_view(forward_strand, reverse_strand, window_size.get() - shape.size() + 1) 
                            | std::views::filter(
                                [scale_factor](uint64_t i)
                                {
                                    return double(i) <= double(__UINT64_MAX__) / scale_factor.get();
                                });
    }
};
 ...
 */
inline constexpr auto fracmin_hash = detail::fracmin_hash_fn{};
